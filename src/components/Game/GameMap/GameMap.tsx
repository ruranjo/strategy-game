...
import { Character, MinaDeOroProperties, AlmacenDeOroProperties, HeroProperties, BuildingProperties } from '../../../types/character.type';
...
const GameMap: React.FC<GameMapProps> = () => {
  ...
  const {heroes,builders,gold, updateResources, selectedCharacter, addGoldMine, setSelectedBuild, setSelectedCharacter, selectedCell, setSelectedCell, selectedBuild, deductGold } = useGameStore((state) => ({
  ...
  const getCellColor = (row: number, col: number): string => {
    ...
    if (cell?.bando === 'enemigo' && cell?.type==='edificio') {
      ...
      const percentage = proper.currentResistance !== undefined && proper.maxResistance !== undefined
  ...
  const startAttack = (attackerCell: { row: number; col: number }, targetCell: { row: number; col: number }, damage: number) => {
    ...
    const intervalId = setInterval(() => {
      ...
      if (!newBoardMatrix[attackerCell.row]?.[attackerCell.col] || !newBoardMatrix[targetCell.row]?.[targetCell.col]) {
      ...
      if (targetProps.currentHealth <= 0) {
        ...
        updatedBoardMatrix[targetCell.row][targetCell.col] = { ...target, properties: { ...targetProps } };
      }
  
      if (attackerProps.currentHealth <= 0) {
        updatedBoardMatrix[attackerCell.row][attackerCell.col] = null;
      } else {
        updatedBoardMatrix[attackerCell.row][attackerCell.col] = { ...attacker, properties: { ...attackerProps } };
      }
  ...
  const startAttackOnBuilding = (
    ...
    const intervalId = setInterval(() => {
      ...
      // Check if either target or attacker is dead to stop the interval
      if (targetProps.currentResistance <= 0 || attackerProps.currentHealth <= 0) {
        clearInterval(intervalId);
  ...
  const handleCellClick = (row: number, col: number) => {
    ...
    if (
      ...
      console.log("rubens 1")
      if (cell && cell.OnAttack) {
        console.log("No se puede seleccionar un personaje mientras está en combate.");
        return;
      }
      ...
      if (adjacentCell) {
        ...
        startAttack({row: adjacentCell.newRow,col:adjacentCell.newCol }, { row, col }, proper.attackDamage);
      }
    }else if(
      selectedCharacter &&
      selectedCharacter.bando === 'jugador' &&
      ...
      if (adjacentCell) {
        ...
        const proper =  auxCharacter.properties as HeroProperties
        startAttackOnBuilding({row: adjacentCell.newRow,col:adjacentCell.newCol }, { row, col }, proper.attackDamage);
      }
      
      
    
    } else if (isAdjacentCellAvailable(row, col)) {
      console.log("rubens 3")
      if (isPushMode && selectedCharacter && selectedBuild) {
        if (boardMatrix[row][col] === null) {
          ...
          console.log("rubens: ",selectedBuild.bando)
          if (
            selectedBuild && 
            selectedBuild.properties && 
            selectedBuild.bando !== 'enemigo' &&
            (selectedBuild.bando !== 'neutro' || selectedCharacter.bando === 'jugador') ) {
  ...
  const handleCellClickWrapper = (rowIndex: number, colIndex: number) => {
    ...
    console.log("cell:", cell,"cell?.type === 'edificio': ",  cell?.type === 'edificio' ,"cell?.type === 'ser': ", cell?.type === 'ser')
    if (cell && (cell.type === 'edificio' || cell.type === 'ser')) {
      handleCellClick(rowIndex, colIndex);
    } else if (selectedCharacter?.type === 'ser' && !selectedBuild) {
      console.log("rubens 5")
